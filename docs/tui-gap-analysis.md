...
## 状态（2026-07-30, branch feat/tui-productization-m1）

| # | 改进项 | 状态 |
|---|------|------|
| 1 | 渲染性能 | ✅ 帧调度 throttle+coalesce（`7117dea`）+ 滚动物化修复（`09c158f`） |
| 2 | 消息组件丰富度 | 🔶 部分：thinking 折叠/耗时、subagent 组显示、工具预览已存在；本次补字符预算折叠（`eb3e92e`）、thinking 耗时（`475c978`）、未闭合 fence（`0a12797`） |
| 3 | 状态栏与头部 | ✅ 持久标题栏（`6896a38`） |
| 4 | 输入区域 | 🔶 行高修复（`531ba83`+`06c128f`）；@mention 高亮不做（Kimi 也未做，见 spec §0） |
| 5 | 流式输出体验 | ✅ 节流 + fence 补全 |
| 6/7 | 动画/终端兼容 | 🔶 VSCode 降帧自适应（`7117dea`） |
