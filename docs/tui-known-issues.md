...
| # | 问题 | 状态 |
|---|------|------|
| 1 | @file 补全在文本中间失效 | ✅ 已修复：`activeMentionAt` 光标感知（`aec8c96`） |
| 2 | 长文本不自动扩展输入行高度 | ✅ 已修复：软换行视觉行计数（`531ba83`） |
| 3 | VSCode PowerShell 历史空白 | 🔶 已缓解：帧调度节流 + VSCode 降帧（`7117dea`），待人工验证 |
| 4 | @file 补全卡顿 | ✅ 已修复：FileIndex 缓存 + fd 委托（`41c82f5`、`a39a284`） |
